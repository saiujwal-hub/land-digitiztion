...
def main():
    ...
    if not public_url:
        print("[TUNNEL] Failed to get Cloudflare tunnel URL.")
        return 1

    print(f"[TUNNEL] Public Cloudflare Web Tunnel Live: {public_url}")
    try:
        TXT_PATH.write_text(public_url, encoding="utf-8")
        (BASE_DIR / "scratch" / "public_url.txt").write_text(public_url, encoding="utf-8")
        print(f"[TUNNEL] Wrote URL to {TXT_PATH.name}")
    except Exception as e:
        print(f"[TUNNEL] Note writing URL file: {e}")
