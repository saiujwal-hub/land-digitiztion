...
def create_exact_ocr_lines():
    lines = [
        # PAGE 1
        OCRLine(text="no12736/2003 100Rs.", score=0.92, x_min=100, y_min=50, x_max=400, y_max=90, page_num=1, page_height=2000),
...
def test_reference_extraction():
    ...
    # 13. stamp_purchase_date & document_date
    assert result["stamp_purchase_date"] == "09-10-2003", f"FAIL stamp_purchase_date: {result['stamp_purchase_date']}"
    assert result["document_date"] is None, f"FAIL document_date should be None: {result['document_date']}"
    print("[PASS] stamp_purchase_date: ", result["stamp_purchase_date"])
